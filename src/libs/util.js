...
export const setUserRole = (role) => {
  Cookies.set('userRole', role, { expires: cookieExpires || 1 })
}
...
export const getMenuByRouter = (list, access) => {
  ...
  forEach(list, item => {
    if (getUserRole() == 1) {
      ...
      if (item.name == '关键词管理' || item.name == '用户管理') {
        return
      }
    }
    if (!item.meta || (item.meta && !item.meta.hideInMenu)) {
      let obj = {
        icon: (item.meta && item.meta.icon) || '', name: item.name, meta: item.meta
      }
...
export const getBreadCrumbList = (route, homeRoute) => {
  ...
  let res = routeMetched.filter(item => {
    ...
    let obj = {
      icon: (item.meta && item.meta.icon) || '', name: item.name, meta: meta
    }
    return obj
  })
  res = res.filter(item => {
